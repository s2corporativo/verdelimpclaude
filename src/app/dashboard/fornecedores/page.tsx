...
export default function FornecedoresPage() {
  ...
  return (<div>
    ...
    <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:16,marginBottom:16}}>
      ...
      <button onClick={salvar} disabled={!form.nome} style={{background:"#4a9410",color:"#fff",border:"none",padding:"9px 24px",borderRadius:8,cursor:"pointer",fontWeight:600}}>+ Cadastrar</button>
    </div>
    <table style={{borderCollapse:"collapse",width:"100%",background:"#fff",borderRadius:12,overflow:"hidden",border:"1px solid #e5e7eb"}}>
      <thead><tr style={{background:"#e8f5ee"}}>{["Nome","CNPJ","Tipo","Município","Situação"].map(h=><th key={h} style={{padding:"9px 12px",textAlign:"left",fontSize:11,fontWeight:700,color:"#334532"}}>{h}</th>)}</tr></thead>

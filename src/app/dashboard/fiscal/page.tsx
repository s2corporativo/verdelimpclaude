  ...
  return (<div>
    ...
    {aba==="despesas"&&(<div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
        {[["Em Aberto","R$"+fmt(despesas.filter((d:any)=>d.status==="em_aberto").reduce((s:number,d:any)=>s+Number(d.totalAmount),0)),"💸","#d97706"],["Pagos","R$"+fmt(despesas.filter((d:any)=>d.status==="pago").reduce((s:number,d:any)=>s+Number(d.totalAmount),0)),"✅","#4a9410"],["Total","R$"+fmt(despesas.reduce((s:number,d:any)=>s+Number(d.totalAmount),0)),"📋","#4a9410"]].map(([l,v,i,c])=>(
    ...
    {aba==="nfse"&&(<div>
      ...
      <table style={{borderCollapse:"collapse",width:"100%",background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,overflow:"hidden"}}>
        ...
        <tbody>{nfses.map((n:any)=><tr key={n.id} style={{borderBottom:"1px solid #f3f4f6"}}>
          ...
          <td style={{padding:"8px 12px",fontWeight:700,color:"#4a9410"}}>R${fmt(Number(n.serviceValue))}</td>
          <td style={{padding:"8px 12px"}}>R${fmt(Number(n.issAmount))}</td>
          <td style={{padding:"8px 12px",fontFamily:"monospace"}}>{n.issRate}%</td>
          <td style={{padding:"8px 12px"}}>{n.issRetained?<span style={{color:"#dc2626",fontWeight:700}}>Sim</span>:<span style={{color:"#4a9410"}}>Não</span>}</td>
    ...
    {aba==="apuracao"&&(<div>
      <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:20,marginBottom:14}}>
        <h3 style={{color:"#334532",fontSize:14,marginBottom:14}}>✨ Gerar lançamentos tributários automaticamente</h3>

...
export default function WhatsAppPage() {
  ...
  return (<div>
    <h1 style={{color:"#334532",fontSize:20,fontWeight:700,marginBottom:4}}>WhatsApp — Alertas Automáticos</h1>
    ...
    <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:18,marginBottom:16}}>
      <h3 style={{color:"#334532",fontSize:14,marginBottom:12}}>⚙️ Configuração</h3>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div style={{background:"#f9fafb",borderRadius:8,padding:12}}>
          <h4 style={{fontSize:12,fontWeight:700,color:"#334532",marginBottom:8}}>Evolution API (Gratuita)</h4>
          {[["WHATSAPP_PROVIDER","evolution"],["EVOLUTION_API_URL","https://seu-servidor:8080"],["EVOLUTION_API_KEY","sua-api-key"],["EVOLUTION_INSTANCE","verdelimp"],["WHATSAPP_ADMIN_NUMBER","5531999990000"]].map(([k,v])=>(
            <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid #f3f4f6",fontSize:10}}>
              <code style={{color:"#4a9410",fontWeight:600}}>{k}</code>
              <span style={{color:"#6b7280"}}>{v}</span>
            </div>
          ))}
          <a href="https://doc.evolution-api.com" target="_blank" rel="noopener noreferrer" style={{display:"block",marginTop:8,color:"#4a9410",fontSize:11,fontWeight:600}}>📖 Documentação Evolution API →</a>
        </div>
        <div style={{background:"#f9fafb",borderRadius:8,padding:12}}>
    ...
    {/* Teste */}
    <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:18,marginBottom:16}}>
      <h3 style={{color:"#334532",fontSize:14,marginBottom:12}}>📱 Testar Envio</h3>
      <div style={{display:"flex",gap:10,alignItems:"flex-end",marginBottom:12}}>
        ...
        <button onClick={testar} disabled={enviando} style={{background:"#4a9410",color:"#fff",border:"none",padding:"9px 22px",borderRadius:8,cursor:"pointer",fontWeight:700,fontSize:13,opacity:enviando?.7:1}}>
    ...
    {alertas.length>0&&(<div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:18,marginBottom:16}}>
      <h3 style={{color:"#334532",fontSize:14,marginBottom:12}}>🔔 Alertas Pendentes ({alertas.length})</h3>
    ...
    <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:16}}>
      <h3 style={{color:"#334532",fontSize:13,marginBottom:10}}>Log ({log.length})</h3>

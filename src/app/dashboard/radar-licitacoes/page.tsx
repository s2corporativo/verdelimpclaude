...
export default function RadarPage() {
  ...
  return(<div>
    <h1 style={{color:"#334532",fontSize:20,fontWeight:700,marginBottom:4}}>Radar de Licitações — PNCP + IA</h1>
    ...
    <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:16,marginBottom:16}}>
      <div style={{display:"flex",gap:10}}>
        ...
        <button onClick={buscarEditais} disabled={loading} style={{background:"#4a9410",color:"#fff",border:"none",padding:"9px 24px",borderRadius:8,cursor:"pointer",fontWeight:700,fontSize:13}}>{loading?"⟳ Buscando...":"🔍 Buscar Editais"}</button>
      </div>
    </div>
    ...
    {itens.map((item:any,i:number)=>{
      ...
      return(<div key={i} style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,padding:16,marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:10,marginBottom:10}}>
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:14,color:"#334532",marginBottom:4}}>{item.objetoCompra||item.descricaoObjeto}</div>
          ...
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{fontSize:18,fontWeight:700,color:"#4a9410"}}>R${fmt(item.valorEstimado||0)}</div>
        ...
        {analise[key]&&<div style={{background:analise[key].includes("SIM")?"#f0fdf4":analise[key].includes("NÃO")?"#fef2f2":"#fffbeb",border:"1px solid "+(analise[key].includes("SIM")?"#86efac":analise[key].includes("NÃO")?"#fca5a5":"#fde68a"),borderRadius:8,padding:"10px 12px",marginBottom:10,fontSize:12,whiteSpace:"pre-wrap",lineHeight:1.6}}>
          <strong style={{color:"#334532"}}>🤖 Análise IA — Verdelimp:</strong><br/>{analise[key]}
        </div>}
        <div style={{display:"flex",gap:8}}>
          ...
          {addedToPipeline[key]
            ...
            : <button onClick={()=>adicionarPipeline(item,i)} style={{background:"#334532",color:"#fff",border:"none",padding:"6px 14px",borderRadius:7,cursor:"pointer",fontSize:11,fontWeight:700}}>🏆 → Pipeline</button>
          }
        </div>
      </div>);
    })}
